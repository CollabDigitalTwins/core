...
describe('computeMarkerLookAt', () => {
  it('targets the marker point exactly', () => {
    const result = computeMarkerLookAt(
      { x: 10, y: 10, z: 10 },
      { x: 0, y: 0, z: 0 },
      { x: 5, y: 2, z: -3 },
      8,
  ...
  it('preserves the offset direction relative to the new marker target', () => {
    const result = computeMarkerLookAt(
    ...
    // Unit direction (0.6, 0.8, 0) * 10 offset from the marker point.
