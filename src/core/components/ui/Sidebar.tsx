...
>(
  (
    ...
    // Get auth state from MenusContext
    const { state, isAuthPage } = React.useContext(MenusContext)
    const { isMobile, sidebarState, openMobile, setOpenMobile } = useSidebar()

    // Translation
    const t = useTranslations('Sidebar')

    // Don't render sidebar at all for auth pages
    if (isAuthPage || state.menus.currentViewer === 'auth') {
      return null
    }
    ...
    return (
      <div
        ref={ref}
        className="group peer text-sidebar-foreground md:block" // removed default hidden class
  ...
  return (
    <Button
      ...
      className={cn(
        '',
        className,
        'opacity-70 hover:opacity-100 transition-opacity duration-200 hover:bg-transparent',
      )}
      {...props}
    >
      <LR.PanelLeft />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        // removed default hidden class
        'absolute inset-y-0 z-20 w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex',
